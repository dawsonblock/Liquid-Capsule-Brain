...
def main():
    ...
    try:
        ...
        assert (
            h_after >= h_before + 1
        ), f"/healthz counter did not increase (before={h_before}, after={h_after})"
        assert (
            s_after >= s_before + 1
        ), f"/state/summary counter did not increase (before={s_before}, after={s_after})"
