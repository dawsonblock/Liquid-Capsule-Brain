"""Self-wiring heuristics for Capsule Brain."""
...
class SelfWirer:
    """Proposes safe, local edits to improve integration and manage memory size."""
    ...
    async def run(self, bus: asyncio.Queue) -> None:
        while not self.engine._shutdown_event.is_set():
            ...
            try:
                ...
                logger.error("Self-wiring error: %s", exc)

    def _propose_edits(self) -> list[dict[str, Any]]:
        metrics = self.engine.iit_analyzer.get_latest_metrics()
        self.performance_history.append(
            {"ts": time.time(), "phi": float(metrics.get("phi", 0.0)), "mem": float(len(self.engine.memory))}
        )
    ...
    def summary(self) -> dict[str, float | int]:
        ...
        avg_phi = sum(entry["phi"] for entry in tail) / len(tail)
