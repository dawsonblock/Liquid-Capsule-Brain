"""Simplified Integrated Information Theory surrogate analysis."""
...
class IITAnalyzer:
    ...
    def _initialize_graph(self) -> None:
        for index in range(8):
            self.graph.add_node(f"n{index}")
        for index in range(7):
            self.graph.add_edge(f"n{index}", f"n{index + 1}")
    ...
    def _compute_phi(self) -> None:
        ...
        glyphs = ["○", "◎", "◉"]
        glyph_index = min(int(phi // 1.5), len(glyphs) - 1)
    ...
    async def run_analysis_loop(self, bus: asyncio.Queue[dict[str, Any]]) -> None:
        while not self.engine._shutdown_event.is_set():
            try:
                if random.random() < 0.2 and self.graph.number_of_nodes() < 300:
                    node_a = f"n{random.randint(0, 9_999)}"
                    node_b = f"n{random.randint(0, 9_999)}"
                    self.graph.add_node(node_a)
                    self.graph.add_node(node_b)
                    self.graph.add_edge(node_a, node_b)
                self._compute_phi()
                await bus.put({"type": "phi_metrics", "payload": self.get_latest_metrics()})
