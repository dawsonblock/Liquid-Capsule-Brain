    ...
    async def _execute_tool(self, hint: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if name == "local_search":
            ...
            return {
                "tool": "local_search",
                "query": q,
                "results": (await retrieve_topk(q)).get("abstracts", []),
            }
        if name == "calculator":
            expr = hint.get("expression", "0")
            return {"tool": "calculator", "expression": expr, "result": _safe_eval(expr)}
