import asyncio
import logging
import time
from typing import Any

log = logging.getLogger(__name__)


async def plan_once(query: str) -> dict[str, Any]:
    start = time.perf_counter()
    try:
        tool_hints: list[dict[str, Any]] = []
        normalized_query = query.lower()

        if any(keyword in normalized_query for keyword in ["latest", "current", "recent", "news"]):
            tool_hints.append({"tool": "local_search", "query": query, "priority": "high"})

        if any(keyword in normalized_query for keyword in ["calculate", "compute", "math"]):
            tool_hints.append({"tool": "calculator", "expression": query, "priority": "medium"})

        await asyncio.sleep(0.05)
        latency_ms = (time.perf_counter() - start) * 1000
        reasoning_steps = "direct_response" if not tool_hints else "tool_assisted"
        return {"tool_hints": tool_hints, "reasoning_steps": reasoning_steps, "confidence": 0.8, "lat_ms": latency_ms}
    except Exception as exc:
        log.error("Planning failed: %s", exc)
        latency_ms = (time.perf_counter() - start) * 1000
        return {"tool_hints": [], "reasoning_steps": "fallback", "lat_ms": latency_ms, "error": str(exc)}
