...
class MockCfC(torch.nn.Module):
    ...
    def forward(
        ...
        for timestep in range(inputs.shape[1]):
            step_input = inputs[:, timestep, :]
            hidden = self.activation(self.l1(step_input) + hidden)
...
class LiquidReasoningCore:
    ...
    def _text_to_embedding(self, text: str) -> torch.Tensor:
        ...
        values = [(hashed >> bit) & 1 for bit in range(INPUT_SIZE)]
    ...
    def _check_state(self) -> bool:
        ...
        hidden_array = self.hidden_state.detach().cpu().numpy()
        if not np.isfinite(hidden_array).all():
            self.hidden_state = torch.zeros(1, HIDDEN_SIZE, device=self.device)
            return False

        if float(np.abs(hidden_array).max()) > 100:
