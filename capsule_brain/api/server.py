...
def _ensure_engine() -> CapsuleEngine:
    if engine is None:
...
async def state_summary() -> dict[str, Any]:
    engine_instance = _ensure_engine()
    return engine_instance.get_state_summary()


@app.post("/ask")
async def ask(q: str) -> dict[str, Any]:
    engine_instance = _ensure_engine()
    engine_instance.add_memory("user", q)
    context, system_prompt = engine_instance.belief_state_manager.synthesize_context_for_llm()
...
async def add_edge(source: str, target: str, relation: str = "related_to") -> dict[str, Any]:
    engine_instance = _ensure_engine()
    engine_instance.add_graph_edge(source, target, relation)
    graph = engine_instance.knowledge_graph
    return {
        "ok": True,
        "graph": {
