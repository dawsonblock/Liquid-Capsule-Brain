"""HTTP API surface for interacting with the Capsule Brain engine."""
...
_engine: CapsuleEngine | None = None


def _require_engine() -> CapsuleEngine:
    if _engine is None:
...
async def on_startup() -> None:
    global _engine
    _engine = CapsuleEngine()
    await _engine.start_background_tasks()
    log.info("Capsule engine started")
...
async def ready() -> dict[str, Any]:
    return {"ready": _engine is not None}
