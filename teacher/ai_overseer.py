...
class AIOverseer:
    def __init__(self, config_path: str, student_api_host: str = "http://localhost:8000"):
        ...
        self.config = cast(dict[str, Any], config["ai_overseer_config"])
    ...
    async def execute_action(self, plan: dict[str, Any]) -> None:
        ...
        if action == "reasoning_probe":
            ...
            edge_payload = {
                "source": plan.get("source") or plan.get("source_node"),
                "target": plan.get("target") or plan.get("target_node"),
                "relation": plan.get("relation", "related_to"),
            }
            if not edge_payload["source"] or not edge_payload["target"]:
                msg = "graph_synthesis action requires 'source' and 'target'"
                raise ValueError(msg)
            await self.http_client.post(
                f"{self.student_api_host}/graph/edge", params=edge_payload
            )
